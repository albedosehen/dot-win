...
class DotWinBloatwareRemoval : DotWinConfigurationItem {
    ...
    [bool] Test() {
        try {
            Write-DotWinLog "Testing if application '$($this.ApplicationName)' exists and needs removal" -Level Verbose
            ...
            if ($appxPackages) {
                $appExists = $true
                Write-DotWinLog "Found AppX packages for '$($this.ApplicationName)': $($appxPackages.Count)" -Level Verbose
            }
            
            # Check provisioned packages
            $provisionedPackages = Get-AppxProvisionedPackage -Online | Where-Object { $_.DisplayName -like "*$($this.ApplicationName)*" }
            if ($provisionedPackages) {
                $appExists = $true
                Write-DotWinLog "Found provisioned packages for '$($this.ApplicationName)': $($provisionedPackages.Count)" -Level Verbose
            }
            
            # Check installed programs
            $installedPrograms = Get-WmiObject -Class Win32_Product | Where-Object { $_.Name -like "*$($this.ApplicationName)*" }
            if ($installedPrograms) {
                $appExists = $true
                Write-DotWinLog "Found installed programs for '$($this.ApplicationName)': $($installedPrograms.Count)" -Level Verbose
            }
            
            return $appExists
            
        } catch {
            Write-DotWinLog "Error testing application '$($this.ApplicationName)': $($_.Exception.Message)" -Level Error
            return $false
        }
    }
    ...
    [void] Apply() {
        try {
            Write-DotWinLog "Removing application '$($this.ApplicationName)'" -Level Information
            
            $removalResults = @()
            
            # Remove AppX packages
            if ('AppxPackage' -in $this.RemovalMethods) {
                $removalResults += Remove-AppxPackages -ApplicationName $this.ApplicationName -PreserveUserData:$this.PreserveUserData
            }
            
            # Remove provisioned packages
            if ('ProvisionedPackage' -in $this.RemovalMethods) {
                $removalResults += Remove-ProvisionedPackages -ApplicationName $this.ApplicationName
            }
            
            # Remove installed programs
            if ('Program' -in $this.RemovalMethods) {
                $removalResults += Remove-InstalledPrograms -ApplicationName $this.ApplicationName
            }
            
            # Remove related services
            if ($this.IncludeServices) {
                $removalResults += Remove-RelatedServices -ApplicationName $this.ApplicationName
            }
            
            # Remove scheduled tasks
            if ($this.IncludeScheduledTasks) {
                $removalResults += Remove-RelatedScheduledTasks -ApplicationName $this.ApplicationName
            }
            
            $successCount = ($removalResults | Where-Object { $_.Success }).Count
            $totalCount = $removalResults.Count
            
            Write-DotWinLog "Application removal completed: $successCount/$totalCount operations successful" -Level Information
            
        } catch {
            Write-DotWinLog "Error removing application '$($this.ApplicationName)': $($_.Exception.Message)" -Level Error
            throw
        }
    }
    ...
    [hashtable] GetCurrentState() {
        try {
            ...
            $installedPrograms = Get-WmiObject -Class Win32_Product -ErrorAction SilentlyContinue | Where-Object { $_.Name -like "*$($this.ApplicationName)*" }
            foreach ($program in $installedPrograms) {
                $state.InstalledPrograms += @{
                    Name = $program.Name
            ...
            $services = Get-Service -Name "*$($this.ApplicationName)*" -ErrorAction SilentlyContinue
            ...
            # Get scheduled tasks
            $tasks = Get-ScheduledTask -TaskName "*$($this.ApplicationName)*" -ErrorAction SilentlyContinue
