...
class DotWinTelemetryConfiguration : DotWinConfigurationItem {
    ...
    [bool] Test() {
        try {
            Write-DotWinLog "Testing telemetry setting '$($this.Name)'" -Level Verbose
            
            $result = switch ($this.SettingType) {
                'Registry' {
                    Test-TelemetryRegistrySetting -Path $this.RegistryPath -Name $this.RegistryName -ExpectedValue $this.RegistryValue
                }
                'Service' {
                    Test-TelemetryServiceSetting -ServiceName $this.ServiceName
                }
                'ScheduledTask' {
                    Test-TelemetryTaskSetting -TaskPath $this.TaskPath -TaskName $this.TaskName
                }
                default {
                    Write-DotWinLog "Unknown telemetry setting type: $($this.SettingType)" -Level Warning
                    $false
                }
            }
            return $result
        } catch {
            Write-DotWinLog "Error testing telemetry setting '$($this.Name)': $($_.Exception.Message)" -Level Error
            return $false
        }
    }
    ...
    [void] Apply() {
        try {
            Write-DotWinLog "Applying telemetry setting '$($this.Name)'" -Level Information
            
            switch ($this.SettingType) {
                'Registry' {
                    Set-TelemetryRegistrySetting -Path $this.RegistryPath -Name $this.RegistryName -Value $this.RegistryValue -Type $this.RegistryType
                }
                'Service' {
                    Set-TelemetryServiceSetting -ServiceName $this.ServiceName
                }
                'ScheduledTask' {
                    Set-TelemetryTaskSetting -TaskPath $this.TaskPath -TaskName $this.TaskName
                }
                default {
            ...
            Write-DotWinLog "Error applying telemetry setting '$($this.Name)': $($_.Exception.Message)" -Level Error
            throw
        }
    }
